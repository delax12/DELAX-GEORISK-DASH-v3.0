 ...
 *
 * SENDER ADDRESS:
 *   Uses Resend's default sandbox sender 'onboarding@resend.dev' until
 *   a custom domain is verified. To send from @delaxcom.com:
 *     1. Resend Dashboard → Domains → Add Domain → delaxcom.com
 *     2. Add the DNS records Resend provides (SPF, DKIM, DMARC)
 *     3. Once verified, change FROM_EMAIL below to 'waitlist@delaxcom.com'
 *
 * ENDPOINT:
 *   POST /api/waitlist
...
// TODO: change to 'DELAX GEO-RISK <waitlist@delaxcom.com>' after domain verification
const FROM_EMAIL   = 'DELAX GEO-RISK <onboarding@resend.dev>';
...
<body style="margin:0;padding:0;background:#0a1020;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;color:#e8edf5">
  ...
  try {
    ...
    if (!resendResp.ok) {
      console.error('[api/waitlist] Resend error:', resendResp.status, body);
      // Don't leak internal details — tell user it was received and log for us
      return res.status(502).json({
        error:  body?.message || `Resend HTTP ${resendResp.status}`,
        detail: body?.name    || null,
      });
    }
